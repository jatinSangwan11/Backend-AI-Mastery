...
def test_revoke_api_key_returns_true_when_key_is_revoked() -> None:
    create_api_key("21", current_time=CURRENT_TIME)
    assert revoke_api_key("1", "21") is True


def test_revoke_api_key_returns_false_for_wrong_user() -> None:
    api_key = create_api_key("user_40", current_time=CURRENT_TIME)

    assert revoke_api_key("1", "user_99") is False
    assert validate_api_key(api_key, CURRENT_TIME) == APIKeyValidationResult(True, "user_40")
...
def test_create_api_key_generates_again_when_key_already_exists() -> None:
    ...
    api_key_directory.add_api_key(
        APIKeyRecord(
            key_id="1",
            api_key_hash=hash_api_key(existing_api_key),
            user_id="user_40",
            created_at=CURRENT_TIME,
            expires_at=CURRENT_TIME + DEFAULT_API_KEY_LIFETIME,
            revoked=False,
    ...
    assert api_key_directory.find_record(hash_api_key("sk-unique-user_40")) == APIKeyRecord(
        key_id="2",
        api_key_hash=hash_api_key("sk-unique-user_40"),
        user_id="user_40",
        created_at=CURRENT_TIME,
        expires_at=CURRENT_TIME + DEFAULT_API_KEY_LIFETIME,
        revoked=False,
